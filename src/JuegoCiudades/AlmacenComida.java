...
public class AlmacenComida implements Runnable {
	...
	public void run() {
		try {
			Thread.sleep(1000);// Esperamos un segundo para generar 10 unidades de comida
			totalComida = totalComida + 10;
			System.out.println("[ALMACEN] Se ha producido "+10+" uds de comida");
	...
	public synchronized boolean solicitarComida(int cantidadComida,String ciudad) {
		try {
			if(cantidadComida<=this.totalComida) {
				...
				totalComida = totalComida - cantidadComida; //restamos del total de comida la comidasolicitada
